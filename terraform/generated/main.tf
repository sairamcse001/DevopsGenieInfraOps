...
resource "aws_lambda_function" "main" {
  ...
  runtime = "nodejs16.x"
 filename   = "lambda.zip" # Placeholder zip file, replace as needed
 memory_size = 128
 timeout     = 30
  s3_bucket  = "my-bucket" # REPLACE with bucket for zip file
  s3_key    = "lambda.zip"  # REPLACE with key for zip file
...
resource "aws_cloudwatch_log_group" "s3_log_group" {
  name = "/aws/s3/${aws_s3_bucket.main.id}"
  retention_in_days = 30
}
